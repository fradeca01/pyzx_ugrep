...
def plot(base_path = "./", save = False):
    ...
    plt.xlabel("k (logical qubits)", fontsize=16)
    plt.ylabel("Time (seconds)", fontsize=16)
    plt.title("Performance with k as variable", fontsize=18)
    plt.legend()
    plt.gca().set_yscale('log')
    plt.grid(True, alpha=0.3)
    if save:
        plt.savefig(base_path + "log_test_k_wograph.png", dpi=300)
    else:
        plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--path", type=str, default="./", help="Path to the benchmark")
    parser.add_argument("--save", action="store_true", help="Save plot")
    args = parser.parse_args()

    plot(args.path, save = args.save)
